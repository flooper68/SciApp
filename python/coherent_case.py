...
def get_coh_angle(options):
    ...
    def angleDep(angle):
        ...
        tsOrder0 = np.abs(T_dd[n_terms, n_terms]) ** 2 * diff_eff_coef_trans(0, angle, n_air, n_grating, wv, period)
        tpOrder0 = np.abs(T_dd[3 * n_terms + 1, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(0, angle, n_air, n_grating,
                                                                                             wv, period)
        ...
        tsOrderNeg1 = np.abs(T_dd[n_terms - 1, n_terms]) ** 2 * diff_eff_coef_trans(-1, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderNeg2 = np.abs(T_dd[n_terms - 2, n_terms]) ** 2 * diff_eff_coef_trans(-2, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderNeg3 = np.abs(T_dd[n_terms - 3, n_terms]) ** 2 * diff_eff_coef_trans(-3, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderNeg4 = np.abs(T_dd[n_terms - 4, n_terms]) ** 2 * diff_eff_coef_trans(-4, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderNeg5 = np.abs(T_dd[n_terms - 5, n_terms]) ** 2 * diff_eff_coef_trans(-5, angle, n_air, n_grating, wv,
                                                                                    period)
        ...
        tsOrderPos1 = np.abs(T_dd[n_terms + 1, n_terms]) ** 2 * diff_eff_coef_trans(1, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderPos2 = np.abs(T_dd[n_terms + 2, n_terms]) ** 2 * diff_eff_coef_trans(2, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderPos3 = np.abs(T_dd[n_terms + 3, n_terms]) ** 2 * diff_eff_coef_trans(3, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderPos4 = np.abs(T_dd[n_terms + 4, n_terms]) ** 2 * diff_eff_coef_trans(4, angle, n_air, n_grating, wv,
                                                                                    period)
        tsOrderPos5 = np.abs(T_dd[n_terms + 5, n_terms]) ** 2 * diff_eff_coef_trans(5, angle, n_air, n_grating, wv,
                                                                                    period)

        Rp_order_neg1 = np.abs(R_ud[3 * n_terms, 3 * n_terms + 1]) ** 2 * diff_eff_coef(-1, angle, n_air, wv, period)
        Rp_order_neg2 = np.abs(R_ud[3 * n_terms - 1, 3 * n_terms + 1]) ** 2 * diff_eff_coef(-2, angle, n_air, wv,
                                                                                            period)
        Rp_order_neg3 = np.abs(R_ud[3 * n_terms - 2, 3 * n_terms + 1]) ** 2 * diff_eff_coef(-3, angle, n_air, wv,
                                                                                            period)
        Rp_order_neg4 = np.abs(R_ud[3 * n_terms - 3, 3 * n_terms + 1]) ** 2 * diff_eff_coef(-4, angle, n_air, wv,
                                                                                            period)
        Rp_order_neg5 = np.abs(R_ud[3 * n_terms - 4, 3 * n_terms + 1]) ** 2 * diff_eff_coef(-5, angle, n_air, wv,
                                                                                            period)

        tpOrderNeg1 = np.abs(T_dd[3 * n_terms, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(-1, angle, n_air, n_grating,
                                                                                            wv, period)
        tpOrderNeg2 = np.abs(T_dd[3 * n_terms - 1, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(-2, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderNeg3 = np.abs(T_dd[3 * n_terms - 2, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(-3, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderNeg4 = np.abs(T_dd[3 * n_terms - 3, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(-4, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderNeg5 = np.abs(T_dd[3 * n_terms - 4, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(-5, angle, n_air,
                                                                                                n_grating, wv, period)
        ...
        tpOrderPos1 = np.abs(T_dd[3 * n_terms + 2, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(1, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderPos2 = np.abs(T_dd[3 * n_terms + 3, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(2, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderPos3 = np.abs(T_dd[3 * n_terms + 4, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(3, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderPos4 = np.abs(T_dd[3 * n_terms + 5, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(4, angle, n_air,
                                                                                                n_grating, wv, period)
        tpOrderPos5 = np.abs(T_dd[3 * n_terms + 6, 3 * n_terms + 1]) ** 2 * diff_eff_coef_trans(5, angle, n_air,
                                                                                                n_grating, wv, period)
    ...
    values = np.array(list(map(angleDep, points)))
    stopTime = time.time()
...
def get_coh_fill(options):
    ...
    values = np.array(list(map(fillleDep, points)))
    stopTime = time.time()

    sent_graphs('Fill factor', points, values, start, end, options, startTime, stopTime)


def get_coh_period(options):
    print('tick_flooper', flush=True)
    n_air = 1
    ...
    def periodDep(period):
        ...
        return Rs_order0, Rs_order_neg1, Rp_order0, Rp_order_neg1

    values = np.array(list(map(periodDep, points)))
...
def get_coh_cap_thick(options):
    ...
    values = np.array(list(map(widthDep, points)))
...
def get_coh_grating_thick(options):
    print('tick_flooper', flush=True)
    n_air = 1
...
def sent_graphs(param, points, values, start, end, options, startTime, stopTime):
    toSend = Message()
