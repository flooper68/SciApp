...
def show_specular_diffraction_ref(values, options, toSend=[], show=True):
    ...
    plt.axis([options['divisions_start'], options['divisions_end'], 0, 1.1 * max])
    plt.legend(loc='upper right')
    plt.grid(True)
    if show:
        plt.show()
    toSend.append(mpld3.fig_to_dict(fig1))
...
def show_specular_diffraction_trans(values, options, toSend=[], show=True):
    ...
    if show:
        plt.show()
    toSend.append(mpld3.fig_to_dict(fig2))
...
def show_total(values, options, toSend=[], show=True):
    ...
    plt.xlabel(options['dependence']['label'])
    plt.axis([options['divisions_start'], options['divisions_end'], 0, 1.1])
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.show()
...
def show_diff_angles(values, options, toSend=[], show=True):
    ...
    plt.grid(True)
    plt.show()
    toSend.append(mpld3.fig_to_dict(fig6))
...
def show_comprehensive(values, options, toSend=[], show=True):
    ...
    toSend.append(mpld3.fig_to_dict(fig1))
    toSend.append(mpld3.fig_to_dict(fig2))
    toSend.append(mpld3.fig_to_dict(fig3))
